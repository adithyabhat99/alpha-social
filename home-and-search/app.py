...
@app.route('/api/v1.0/gethome/<num>')
@token_required
def home(userid,num):
    #num=0 for first 20 posts,num=1 for 20 to 40 and so on...
    URL='http://localhost:7800/api/v1.0/getfollowinglist/{0}'.format(userid)
    response=requests.get(url=URL,headers={"x-access-token":request.headers["x-access-token"]})
    ...
    users="'"+users+"'"
    query="select * from posts.post where userid in ({0}) order by date desc limit {1},{2}".format(users,base,top)
